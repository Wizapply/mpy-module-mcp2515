...
static mp_obj_t can_init(size_t n_args, const mp_obj_t *args)
{
    ...
    if (MCP_reset() != ERROR_OK)
        return mp_const_false;
    if (MCP_setBitrate(can_baudrate_config, can_clock_config) != ERROR_OK)
        return mp_const_false;
...
static mp_obj_t can_filter(mp_obj_t idx, mp_obj_t filter, mp_obj_t ext)
{
    if (mp_sv_SPI_obj == mp_const_none)
        return mp_const_false;

    uint32_t idx_int = (uint32_t)mp_obj_get_int(idx);
    uint32_t filter_int = (uint32_t)mp_obj_get_int(filter);
    bool ext_b = mp_obj_get_int(ext) != 0;
    if (idx_int < 0 || idx_int > 5)
    {
        return mp_const_false;
    }
    if (MCP_setFilter((MCP_RXF)(RXF0 + idx), ext_b, filter_int) != ERROR_OK)
    {
        return mp_const_false;
    }
    return mp_const_true;
...
static mp_obj_t can_mask(mp_obj_t idx, mp_obj_t mask, mp_obj_t ext)
{
    if (mp_sv_SPI_obj == mp_const_none)
        return mp_const_false;
    ...
    bool ext_b = mp_obj_get_int(ext) != 0;
    if (idx_int < 0 || idx_int > 1)
    {
        return mp_const_false;
    }

    if (MCP_setFilterMask(idx_int == 0 ? MASK0 : MASK1, ext_b, mask_int) != ERROR_OK)
    {
        return mp_const_false;
    }
    return mp_const_true;
}
static MP_DEFINE_CONST_FUN_OBJ_3(can_mask_obj, can_mask);
...
mp_obj_t mpy_init(mp_obj_fun_bc_t *self, size_t n_args, size_t n_kw, mp_obj_t *args)
{
    ...
    mp_store_global(MP_QSTR_can_recv_polling, MP_OBJ_FROM_PTR(&can_recv_polling_obj));
    ...
    mp_store_global(MP_QSTR_can_mask, MP_OBJ_FROM_PTR(&can_mask_obj));
